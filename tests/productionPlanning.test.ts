...
test('Production Planning reuses the existing menu and supports multi-order planning', async () => {
  ...
  assert.equal((component.match(/className="production-required-mark"/g) ?? []).length, 9)
  ...
  assert.match(component, /aria-label="Cut Length in CM"[^\n]+required/)
  assert.match(component, /!Number\.isFinite\(v\.twoPlyQuantity\)/)
  assert.match(component, /Number\(v\.deckleSize\) <= 0/)
  assert.match(component, /value=\{v\.twoPlyQuantity \?\? ''\}/)
  assert.match(component, /aria-label="Deckle Size"/)
  assert.match(component, /aria-label="Deckle Size" readOnly=\{isCalculated\}/)
  assert.match(component, /aria-label="Cut Length in CM" readOnly=\{isCalculated\}/)
...
test('Production Plan persistence is additive, audited, and concurrency guarded', async () => {
  ...
  assert.match(plannedComponent, /colSpan=\{33\}/)
