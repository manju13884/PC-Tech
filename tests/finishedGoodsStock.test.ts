...
test('Finished Goods Stock preserves Job Card linkage and derives dispatch balances',async()=>{
  ...
  assert.match(api,/finished_goods_balances/)
  assert.doesNotMatch(api,/dispatch_name/)
  assert.match(component,/quantity\(row\.dispatched_quantity\)/)
  assert.match(component,/quantity\(row\.closing_stock\)/)
  ...
  for(const label of ['SO','Customer','Item & Description','Job Card No.','Manufactured Date','Manufactured Qty','Dispatched Qty','Stock Adjustment','Closing Stock'])assert.ok(component.includes(label))
  assert.match(component,/Export Excel/)
  assert.doesNotMatch(component,/>Edit<|>Delete</)
  assert.match(component,/row.closing_stock<=0/)
  assert.match(component,/FinishedGoodsDispatch/)
