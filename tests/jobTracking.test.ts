...
test('Job Tracking is a permission-controlled ERP workflow backed by Job Cards', async () => {
  ...
  assert.match(api, /trackedJobsAndReels\(db, user\)/)
  assert.match(api, /Supervisor is assigned from the logged-in user and is read-only/)
  ...
  assert.match(component, /supervisorReadOnly defaultSupervisorName=\{currentUser\.fullName\}/)
  ...
  assert.match(jobCardComponent, /line\.supervisor_name \|\| defaultSupervisorName/)
  assert.match(jobCardComponent, /aria-label=\{label\} value=\{line\.supervisor_name \|\| defaultSupervisorName\} readOnly/)
  ...
  assert.match(component, /job-tracking-expand is-\$\{job\.job_status\.toLowerCase\(\)\}/)
  assert.match(styles, /\.job-tracking-expand \{[^}]*width: 20px[^}]*height: 20px[^}]*margin: 0 auto/s)
  assert.match(styles, /\.job-tracking-expand\.is-created[^}]*background:#3b82f6/)
  assert.match(styles, /\.job-tracking-expand\.is-in_progress[^}]*background:#f59e0b/)
  assert.match(styles, /\.job-tracking-expand\.is-completed[^}]*background:#22a35a/)
  assert.match(styles, /\.job-tracking-expand\.is-cancelled[^}]*background:#ef4444/)
  assert.doesNotMatch(styles, /\.job-tracking-expand \{[^}]*linear-gradient/s)
  assert.match(styles, /tr\.is-expanded \.job-tracking-expand \{[^}]*filter:brightness\(\.84\)/s)
