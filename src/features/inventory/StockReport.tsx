...
export default function StockReport({ username, userRole = "" }: { username: string; userRole?: string }) {
  ...
  const [transactions, setTransactions] = useState<StockTransaction[] | null>(
    null,
  );
  ...
  const load = async (next = filters) => {
    ...
    setError("");
    try {
      setReport(await loadStockReport(next));
    } catch (e) {
      setError(e instanceof Error ? e.message : "Unable to load Stock Report.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const viewTransactions = async (row: StockReportRow) => {
    setBusy(true);
    setError("");
    try {
      setTransactions(await loadStockTransactions(row.id, filters.as_on_date));
      setTransactionTitle(`${row.material_no} | ${row.reel_number}`);
    } catch (e) {
  ...
  const confirmDelete = async () => {
    if (!deleteRow || !deleteReason.trim() || deleting) return;
    setDeleting(true);
    setError("");
    setNotice("");
    try {
      const message = await deleteMaterialStock(
        deleteRow.id,
        deleteReason.trim(),
      );
      setDeleteRow(null);
      ...
      setError(
        e instanceof Error
          ? e.message
          : "Unable to delete the Material Stock row.",
      );
    } finally {
      setDeleting(false);
    }
  ...
  const exportExcel = async () => {
    ...
    setError("");
    try {
      const exported = await loadStockReport({
        ...filters,
        export: "1",
        page: "1",
        page_size: "10000",
      ...
      const rowXml = rows
        .map(
          (row) =>
            `<Row>${row.map((cell, index) => `<Cell><Data ss:Type="${numeric.has(index) && cell !== "" ? "Number" : "String"}">${esc(cell)}</Data></Cell>`).join("")}</Row>`,
        )
        .join("");
      const metadata = [
        `<Row><Cell><Data ss:Type="String">Stock Report</Data></Cell></Row>`,
        `<Row><Cell><Data ss:Type="String">As On Date</Data></Cell><Cell><Data ss:Type="String">${esc(exported.asOnDate)}</Data></Cell></Row>`,
        `<Row><Cell><Data ss:Type="String">Generated Date/Time</Data></Cell><Cell><Data ss:Type="String">${esc(formatIstDateTime(new Date()))}</Data></Cell></Row>`,
        `<Row><Cell><Data ss:Type="String">Generated By</Data></Cell><Cell><Data ss:Type="String">${esc(username)}</Data></Cell></Row>`,
      ].join("");
      ...
      link.download = `Stock-Report-${exported.asOnDate}.xls`;
      link.click();
      URL.revokeObjectURL(link.href);
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "Unable to export Stock Report.",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="stock-report-page">
      {error && (
        <p className="stock-report-message" role="alert">
          {error}
        </p>
      )}
      {notice && <p className="stock-report-message success" role="status">{notice}</p>}
      <section className="stock-report-filters">
        {[
          ["as_on_date", "As On Date", "date"],
          ["reel_number", "Reel / Lot No.", "text"],
          ["gsm", "GSM", "number"],
          ["bf", "BF", "number"],
          ["reel_size", "Reel Size", "number"],
        ...
        <label>
          ...
          <select
            ...
            onChange={(e) => change("material_type", e.target.value)}
          >
        ...
        <label>
          <span>Material</span>
          <input
            placeholder="Material no. or name"
        ...
        <label>
          ...
          <input
            ...
            onChange={(e) => change("paper_type", e.target.value)}
          />
        </label>
        <label>
          <span>Color</span>
        ...
        <label>
          ...
          <select
            ...
            onChange={(e) => change("stock_status", e.target.value)}
          >
            <option value="">All</option>
        ...
        <label>
          ...
          >
            ...
            <option value="reel_size">Reel Size</option>
        ...
        <div className="stock-report-actions">
          <button onClick={() => void load(filters)} disabled={busy}>
            Apply Filters
          </button>
          <button
            className="secondary"
      ...
      <section className="stock-report-summary">
        ...
        {report.totals.map((v) => (
          <article key={v.uom}>
            <span>Total Stock ({v.uom})</span>
            <strong>{n(v.total_stock)}</strong>
          </article>
        ))}
        <article>
          <span>In Stock (No. of Reels)</span>
          <strong>
            {report.totals.reduce((a, v) => a + Number(v.in_stock), 0)}
          </strong>
        </article>
        <article>
          <span>Zero Stock (No. of Reels)</span>
          <strong>
            {report.totals.reduce((a, v) => a + Number(v.zero_stock), 0)}
          </strong>
        </article>
        <article>
          <span>Low Stock</span>
      ...
      <section className="stock-report-grid">
        <div className="stock-report-scroll">
          <table>
            <thead>
              <tr>
                {[
            ...
            <tbody>
              {report.rows.map((v, i) => (
                ...
                >
                  ...
                  <td>
                    <span
                      className={`stock-reel-status is-${v.reel_status.toLowerCase()}`}
                    >
                      {v.reel_status}
                    </span>
                  </td>
                  <td>{v.reserved_for_job || "-"}</td>
                  <td>{v.paper_type || "-"}</td>
                  <td>{v.gsm ?? "-"}</td>
                  <td>{v.bf ?? "-"}</td>
                  ...
                  <td>
                    <span className="stock-report-row-actions">
                      ...
                      <button
                        ...
                        title={v.can_delete ? "Delete unused stock row" : "This row cannot be deleted because it is mapped, used, or you do not have delete access"}
                        onClick={() => {
                          setDeleteRow(v); setDeleteReason(""); setError(""); setNotice("");
                        }}
                      >
                        <Trash2 size={13} />
                      </button>
        ...
        <footer>
          ...
          <div>
            <button
              ...
              onClick={() => {
                const next = { ...filters, page: String(report.page - 1) };
                setFilters(next);
                void load(next);
              }}
            >
      ...
      {transactions && (
        <div className="stock-report-modal" role="dialog" aria-modal="true">
          <section>
            <header>
              <div>
              ...
              <table>
                ...
                <tbody>
                  {transactions.map((v, i) => (
                    <tr key={`${v.transaction_date}-${i}`}>
                      <td>{date(v.transaction_date)}</td>
                      <td>{v.transaction_type}</td>
                      <td>
                        {v.source === "Job Consumption" ? (
                          <a
                            href="#job-tracking"
                            title={`Open Job ${v.reference_number}`}
                          >
                            {v.reference_number}
                          </a>
                        ) : (
        ...
        <div className="stock-report-modal stock-delete-modal" role="dialog" aria-modal="true" aria-labelledby="delete-material-title">
          <section>
            ...
            <div className="stock-delete-body">
              <p>This row can be deleted only because it has not been mapped to or consumed by a Job Card and has no stock transactions.</p>
              <label>
                <span>Reason for deletion <b aria-label="required">*</b></span>
                <textarea maxLength={500} rows={4} value={deleteReason} onChange={(event) => setDeleteReason(event.target.value)} placeholder="Enter reason for deleting this Material Stock row" autoFocus />
              </label>
              <small>{deleteReason.length}/500</small>
            </div>
            <footer>
