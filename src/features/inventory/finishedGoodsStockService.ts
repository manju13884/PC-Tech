export interface FinishedGoodsRow{
  ...
  polar_canvas_item_code:string|null;job_number:string;manufactured_date:string;manufactured_quantity:number;dispatched_quantity:number;stock_adjustment:number;closing_stock:number;uom:string
}
export interface FinishedGoodsCustomer{customer_id:string;customer_name:string}
export interface FinishedGoodsResult{rows:FinishedGoodsRow[];customers:FinishedGoodsCustomer[];canDispatch?:boolean;canAdjust?:boolean}
...
export async function loadFinishedGoodsStock(filters:Record<string,string>):Promise<FinishedGoodsResult>{
  ...
  return{rows:Array.isArray(value.rows)?value.rows:[],customers:Array.isArray(value.customers)?value.customers:[],canDispatch:value.canDispatch===true,canAdjust:value.canAdjust===true}
}
