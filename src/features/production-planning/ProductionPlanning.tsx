...
import { dimensionsForProductionLine, initializeProductionMachine, updateProductionUps } from './productionMachineDimensions'
...
export default function ProductionPlanning() {
  ...
  const renderPlanningRow = (v: PlanLine, index: number) => {
    ...
    return <tr key={`${v.salesOrderId}-${v.lineItemId}`} className={v.productionStatus === 'SPECIFICATION_MISSING' ? 'is-missing' : ''}>
      ...
      <td>{v.productType || '—'}</td><td>{v.ply ? `${v.ply} Ply` : '—'}</td><td>{fluteRun}</td><td className="machine-ups-cell"><input aria-label="Ups" className="production-quantity production-ups" type="number" min="1" step="1" required value={v.ups ?? ''} disabled={!v.included} aria-invalid={v.included && parseProductionUps(v.ups) == null} title="Ups must be a positive whole number" onChange={(e) => { const ups = e.target.value === '' ? null : Number(e.target.value); setLines((all) => all.map((x) => x.salesOrderId === v.salesOrderId && x.lineItemId === v.lineItemId ? updateProductionUps(x, ups) : x)) }} /></td><td className="machine-deckle-cell"><input aria-label="Deckle Size" title="Deckle Size in CM" className="production-quantity" required type="text" value={v.deckleSize ?? preloadedDeckleSize(v)} disabled={!v.included} onBlur={() => setLines((all) => all.map((x) => x.salesOrderId === v.salesOrderId && x.lineItemId === v.lineItemId && !isCalculated && !x.deckleSize.trim() ? { ...x, deckleSize: calculatedDeckleSize(x) } : x))} onChange={(e) => { const deckleSize = e.target.value; setLines((all) => all.map((x) => x.salesOrderId === v.salesOrderId && x.lineItemId === v.lineItemId ? { ...x, deckleSize } : x)) }} /></td><td className="machine-cut-length-cell"><input aria-label="Cut Length in CM" title="Cut Length in CM" className="production-quantity" type="number" min="0.001" step="any" required value={v.cutLengthCm ?? ''} disabled={!v.included} onBlur={() => setLines((all) => all.map((x) => x.salesOrderId === v.salesOrderId && x.lineItemId === v.lineItemId && !isCalculated && x.cutLengthCm == null ? { ...x, cutLengthCm: calculatedCutLengthCm(x.lengthMm, x.widthMm) } : x))} onChange={(e) => { const cutLengthCm = e.target.value === '' ? null : Number(e.target.value); setLines((all) => all.map((x) => x.salesOrderId === v.salesOrderId && x.lineItemId === v.lineItemId ? { ...x, cutLengthCm } : x)) }} /></td>
