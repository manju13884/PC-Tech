...
const CardBoxCalculator: React.FC = () => {
  ...
  const [cFluteGsm, setCFluteGsm] = useState<N>(120);
  const [cFluteBF, setCFluteBF] = useState<N>(16);
  const [cLinerGsm, setCLinerGsm] = useState<N>(120);
  const [cLinerBF, setCLinerBF] = useState<N>(16);
  const [aFluteGsm, setAFluteGsm] = useState<N>(120);
