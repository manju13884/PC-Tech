...
import { isTwoPlyRoll, twoPlyRollLayerName } from '../production-planning/twoPlyRollComposition'
...
export default function ProductSpecifications() {
  ...
  const twoPlyRoll = isTwoPlyRoll(item?.item_name, item?.description, form.product_name)
  const showsPaperComposition = showsBoardFields || twoPlyRoll
  ...
  const startAdd = () => {
    ...
    const selectedItem = items.find((value) => value.item_id === listItemId)
    const specificationType = detectType(selectedItem)
    const selectedIsTwoPlyRoll = isTwoPlyRoll(selectedItem?.item_name, selectedItem?.description)
    setEditingId(null); setReadOnlyMode(false); setCustomerId(listCustomerId); setItemId(listItemId); setForm({ ...emptyForm, polar_canvas_item_code: nextItemCode(), specification_type: specificationType, ...(selectedIsTwoPlyRoll ? { ply: '2', paper_layers: buildPaperLayers('2', []) } : {}), production_stages: defaultProductionStages(specificationType, false) }); setMessage(''); setError(''); setShowForm(true)
  }
  ...
  return <form className="coc-form product-spec-form" onSubmit={save}>
    ...
    {customer && item && <section className="product-spec-panel product-spec-technical">
      ...
      <div className="product-spec-grid">
        ...
        {(showsDimensions || showsFlexibleFields || showsRollFields) && <label>{twoPlyRoll ? 'Deckle / Width (cm)' : showsRollFields ? 'Deckle / Width (mm)' : 'Width - OD (mm)'}<input className="spec-dimension-input" type="number" min="0" step="0.01" value={twoPlyRoll && form.width_mm !== '' ? String(Number(form.width_mm) / 10) : form.width_mm} onChange={(e) => update('width_mm', twoPlyRoll && e.target.value !== '' ? String(Number((Number(e.target.value) * 10).toFixed(8))) : e.target.value)} /></label>}
      ...
      {isPartition && <section className="partition-configuration">
        ...
        <div className="partition-subsection"><strong>Overall Dimensions</strong><div className="partition-field-grid partition-three-columns">
          ...
          <label><span className="spec-field-label">Width (mm) <b className="spec-required-mark" aria-label="required">*</b></span><input className="spec-dimension-input" required type="number" min="0.01" step="0.01" value={twoPlyRoll && form.width_mm !== '' ? String(Number(form.width_mm) / 10) : form.width_mm} onChange={(e) => update('width_mm', twoPlyRoll && e.target.value !== '' ? String(Number((Number(e.target.value) * 10).toFixed(8))) : e.target.value)} /></label>
      ...
      {showsPaperComposition && <div className="paper-composition">
        <div className="paper-composition-heading"><strong>Paper Composition{form.ply ? ` · ${form.ply} Ply` : ''}</strong><span>{form.paper_layers.length > 0 ? form.paper_layers.map((layer) => twoPlyRoll ? twoPlyRollLayerName(layer.layer_name, true) : paperLayerDisplayName(layer.layer_name, isPartition, form.flute_type)).join(' + ') : isPartition ? 'Select Partition Ply above to enter paper composition.' : 'Select Ply above to enter paper composition.'}</span></div>
        {form.paper_layers.length > 0 && <div className="paper-composition-table"><table><thead><tr><th>#</th><th>Layer Type</th><th>GSM</th><th>BF/RCT</th><th>{twoPlyRoll ? 'Deckle Size (cm)' : 'Deckle Size'}</th><th>Shade</th><th>Paper Grade</th><th>Flute</th></tr></thead><tbody>
          {form.paper_layers.map((layer, index) => {
            const isFluting = layer.layer_name.toLowerCase().includes('fluting')
            return <tr key={`${layer.layer_name}-${index}`}><td>{index + 1}</td><td><strong>{twoPlyRoll ? twoPlyRollLayerName(layer.layer_name, true) : paperLayerDisplayName(layer.layer_name, isPartition, form.flute_type)}</strong></td><td><input type="number" min="0" step="0.01" value={layer.gsm} onChange={(e) => updatePaperLayer(index, 'gsm', e.target.value)} placeholder="GSM" /></td><td><input type="number" min="0" step="0.01" value={layer.bf_rct} onChange={(e) => updatePaperLayer(index, 'bf_rct', e.target.value)} placeholder="BF / RCT" /></td><td><input type="text" value={layer.deckle_size || calculatedDeckle(form)} onChange={(e) => updatePaperLayer(index, 'deckle_size', e.target.value)} placeholder={twoPlyRoll ? 'Deckle size (cm)' : 'Deckle size'} /></td><td><select value={layer.shade} onChange={(e) => updatePaperLayer(index, 'shade', e.target.value)}><option value="">Select</option><option value="GYT">GYT</option><option value="Natural">Natural</option><option value="White">White</option></select></td><td><input value={layer.paper_grade} onChange={(e) => updatePaperLayer(index, 'paper_grade', e.target.value)} placeholder={isFluting ? 'Fluting medium' : 'Kraft liner'} /></td><td>{isFluting ? <select value={layer.flute} onChange={(e) => updatePaperLayer(index, 'flute', e.target.value)}><option value="">Select</option><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="E">E</option><option value="F">F</option></select> : <span className="not-applicable">—</span>}</td></tr>
          })}
        </tbody></table></div>}
      </div>}
      ...
      <div className="product-spec-report-dialog">
        ...
        <div className="product-spec-report-scroll">
          <article className="product-spec-report-a4">
            ...
            {form.paper_layers.length > 0 && <section><h2>Paper Composition</h2><table><thead><tr><th>#</th><th>Layer Type</th><th>GSM</th><th>BF/RCT</th><th>{twoPlyRoll ? 'Deckle Size (cm)' : 'Deckle Size'}</th><th>Shade</th><th>Paper Grade</th><th>Flute</th></tr></thead><tbody>{form.paper_layers.map((layer, index) => <tr key={`${layer.layer_name}-report`}><td>{index + 1}</td><td>{twoPlyRoll ? twoPlyRollLayerName(layer.layer_name, true) : paperLayerDisplayName(layer.layer_name, isPartition, form.flute_type)}</td><td>{layer.gsm || '—'}</td><td>{layer.bf_rct || '—'}</td><td>{layer.deckle_size || calculatedDeckle(form)}</td><td>{layer.shade || '—'}</td><td>{layer.paper_grade || '—'}</td><td>{layer.flute || '—'}</td></tr>)}</tbody></table></section>}
