  ...
  return <form className="coc-form product-spec-form" onSubmit={save}>
    ...
    {customer && item && <section className="product-spec-panel product-spec-technical">
      ...
      <div className="product-spec-grid">
        ...
        {showsDimensions && <label>Length (mm)<input className="spec-dimension-input" type="number" min="0" step="0.01" value={form.length_mm} onChange={(e) => update('length_mm', e.target.value)} /></label>}
        {(showsDimensions || showsFlexibleFields || showsRollFields) && <label>{showsRollFields ? 'Deckle / Width (mm)' : 'Width (mm)'}<input className="spec-dimension-input" type="number" min="0" step="0.01" value={form.width_mm} onChange={(e) => update('width_mm', e.target.value)} /></label>}
        {showsHeight && <label>Height (mm)<input className="spec-dimension-input" type="number" min="0" step="0.01" value={form.height_mm} onChange={(e) => update('height_mm', e.target.value)} /></label>}
        ...
        <div className="product-spec-report-scroll">
          <article className="product-spec-report-a4">
            ...
            <section><h2>Technical Specification</h2><dl className="spec-report-details">
              <div><dt>Length</dt><dd>{form.length_mm ? `${form.length_mm} mm` : '—'}</dd></div><div><dt>Width</dt><dd>{form.width_mm ? `${form.width_mm} mm` : '—'}</dd></div><div><dt>Height</dt><dd>{form.height_mm ? `${form.height_mm} mm` : '—'}</dd></div><div><dt>Ply</dt><dd>{form.ply ? `${form.ply} Ply` : '—'}</dd></div>
