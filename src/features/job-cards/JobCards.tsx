...
export interface JobCardLine {
  ...
  supervisor_name?: string | null; quality_name?: string | null; dispatch_name?: string | null
  box_weight_kg?: number | null; manufactured_quantity?: number | null
}
...
interface ProcessEntry {
  process_name: string; start_datetime: string | null; end_datetime: string | null
  in_quantity: number | null; out_quantity: number | null; employee_name: string | null
  in_quantity_2: number | null; out_quantity_2: number | null; employee_name_2: string | null
  reel_number: string | null; in_reel_weight: number | null; out_reel_weight: number | null; remaining_reel_weight: number | null
  reel_number_2: string | null; in_reel_weight_2: number | null; out_reel_weight_2: number | null; remaining_reel_weight_2: number | null
}
...
interface JobCardAttributes {
  paper_layers?: PaperLayer[]; production_stages?: string[]; board_type?: string; joint_type?: string
  material?: string; flute_type?: string; paper_type?: string; shade_color?: string; finish?: string; board_creasing_allowance?: string
}
const readAttributes = (line: JobCardLine): JobCardAttributes => {
  try { return JSON.parse(line.attributes_json || '{}') as JobCardAttributes }
  catch { return {} }
}
const calculated = (line: JobCardLine) => {
  ...
  return { layers, deckle, rotary, rotaryCalculation, slottingSize, boxWeight, bs }
}
...
function ReelDetailCell({ stage, label, entry, field, secondField, numeric = false, processEditable, savingProcessKey, onChange }: { stage: string; label: string; entry?: ProcessEntry; field: ProcessField; secondField: ProcessField; numeric?: boolean; processEditable: boolean; savingProcessKey: string; onChange?: (processName: string, field: ProcessField, value: string) => void }) {
  ...
  const input = (inputField: ProcessField, suffix: string) => <input key={`${stage}:${inputField}:${entry?.[inputField] ?? ''}`} className="job-card-process-entry" aria-label={`${stage} ${label}${suffix}`} inputMode={numeric ? 'decimal' : undefined} maxLength={numeric ? undefined : 80} defaultValue={entry?.[inputField] ?? ''} disabled={savingProcessKey === `${stage}:${inputField}`} onBlur={(event) => onChange?.(stage, inputField, event.target.value)} />
  return <td className={hasSecondReel ? 'job-card-split-process-values' : ''}><div className={hasSecondReel ? 'job-card-process-entry-stack' : ''}>{processEditable ? <>{input(field, hasSecondReel ? ' 1' : '')}{hasSecondReel && input(secondField, ' 2')}</> : <><span>{display(entry?.[field])}</span>{hasSecondReel && <span>{display(entry?.[secondField])}</span>}</>}</div></td>
}
...
export default function JobCards() {
  ...
  return <div className="production-planning-workspace production-planned-workspace job-cards-workspace">
    ...
    <div className="production-planned-toolbar job-cards-toolbar">
      ...
      <div className="production-planned-controls job-cards-actions">
        ...
        {view === 'ready' ? <button className="job-cards-primary" type="button" disabled={!selected.length || busy} onClick={createCards}><ClipboardPlus size={14} />{busy ? 'Creating...' : `Create Job Cards (${selected.length})`}</button>
          : <button className="job-cards-primary job-cards-print" type="button" disabled={!selectedLines.length} onClick={printSelected}><Printer size={14} /> Print Selected ({selectedLines.length})</button>}
      </div>
    </div>
    <section className="production-selection-panel production-planned-grid-panel job-cards-list"><div className="job-cards-table-scroll"><table><colgroup><col className="job-grid-select" /><col className="job-grid-id" /><col className="job-grid-status" /><col className="job-grid-date" /><col className="job-grid-plan" /><col className="job-grid-order" /><col className="job-grid-item" /><col className="job-grid-customer" /><col className="job-grid-product-name" /><col className="job-grid-product-description" /><col className="job-grid-qty" /></colgroup><thead><tr className="job-cards-groups"><th rowSpan={2}><input aria-label="Select all" type="checkbox" checked={filtered.length > 0 && filtered.every((line) => selected.includes(line.production_plan_line_id))} onChange={() => setSelected(filtered.every((line) => selected.includes(line.production_plan_line_id)) ? [] : filtered.map((line) => line.production_plan_line_id))} /></th><th className="group-job" colSpan={2}>Job Card</th><th className="group-schedule" colSpan={2}>Schedule</th><th className="group-order" colSpan={2}>Order &amp; Item</th><th className="group-customer">Customer</th><th className="group-product" colSpan={2}>Product</th><th className="group-production">Production</th></tr><tr><th>Job ID</th><th>Status</th><th>Production Date</th><th>Plan</th><th>Sales Order</th><th>PC Item Code</th><th>Customer</th><th>Product Name</th><th>Product Description</th><th>Box Qty</th></tr></thead><tbody>
      {filtered.map((line) => <tr key={line.production_plan_line_id} className={selected.includes(line.production_plan_line_id) ? 'is-selected' : ''}><td><input aria-label={`Select ${line.job_number || line.sales_order_number}`} type="checkbox" checked={selected.includes(line.production_plan_line_id)} onChange={() => toggle(line.production_plan_line_id)} /></td><td><strong>{line.job_number || 'Not created'}</strong></td><td><span className={line.job_card_id ? 'created' : 'ready'}>{line.job_status || 'READY'}</span></td><td>{formatIstDate(line.plan_date)}</td><td>{line.plan_number}</td><td><strong>{line.sales_order_number}</strong></td><td><button className="job-card-specification-link" type="button" disabled={!line.specification_code} onClick={() => setViewingSpecification(line)}>{line.specification_code || 'Not mapped'}</button></td><td>{line.customer_name}</td><td className="job-cards-product">{line.product_name}</td><td className="job-cards-product">{line.item_description || line.item_name}</td><td className="numeric job-cards-box-qty">{numberText(line.production_quantity)}</td></tr>)}
...
function ProductSpecificationDialog({ line, onClose }: { line: JobCardLine; onClose: () => void }) {
  ...
  return <div className="job-card-specification-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="job-card-specification-dialog" role="dialog" aria-modal="true" aria-labelledby="job-card-specification-title" onMouseDown={(event) => event.stopPropagation()}>
      <header><div><strong id="job-card-specification-title">Product Specification</strong><span>{line.specification_code}</span></div><button type="button" aria-label="Close Product Specification" onClick={onClose}><X size={17} /></button></header>
      <div className="job-card-specification-body">
        <dl className="job-card-specification-summary"><div><dt>Customer</dt><dd>{line.customer_name}</dd></div><div><dt>Product Name</dt><dd>{line.product_name || ''}</dd></div><div><dt>Item</dt><dd>{line.item_name}</dd></div><div><dt>Design Type</dt><dd>{line.product_type}</dd></div><div><dt>Outer Dimensions (MM)</dt><dd>{[line.length_mm, line.width_mm, line.height_mm].filter((value) => value != null).join(' X ')}</dd></div><div><dt>Ply</dt><dd>{line.ply ? `${line.ply} Ply` : ''}</dd></div><div><dt>Material</dt><dd>{attributes.material || ''}</dd></div><div><dt>Flute</dt><dd>{attributes.flute_type || ''}</dd></div><div><dt>Joint</dt><dd>{attributes.joint_type || ''}</dd></div><div><dt>Board Type</dt><dd>{attributes.board_type || ''}</dd></div><div><dt>Board / Creasing Allowance</dt><dd>{attributes.board_creasing_allowance || ''}</dd></div><div><dt>Print</dt><dd>{line.print_required ? 'Required' : 'Not Required'}</dd></div><div><dt>Print Colors</dt><dd>{line.print_colors || ''}</dd></div></dl>
        <h3>Paper Composition</h3>
        <table><thead><tr><th>#</th><th>Layer</th><th>Paper Grade</th><th>GSM</th><th>BF / RCT</th><th>Shade</th><th>Flute</th><th>Deckle Size</th></tr></thead><tbody>{layers.map((layer, index) => <tr key={`${layer.layer_name}-${index}`}><td>{index + 1}</td><td>{layer.layer_name}</td><td>{layer.paper_grade}</td><td>{layer.gsm}</td><td>{layer.bf_rct}</td><td>{layer.shade}</td><td>{layer.flute}</td><td>{layer.deckle_size}</td></tr>)}{!layers.length && <tr><td colSpan={8}>Paper composition not available.</td></tr>}</tbody></table>
        <h3>Production Stages</h3><ol className="job-card-specification-stages">{stages.map((stage) => <li key={stage}>{stage}</li>)}</ol>
        {line.specification_notes && <><h3>Specification Notes</h3><p className="job-card-specification-notes">{line.specification_notes}</p></>}
      </div>
    </section>
  </div>
}

export function JobCard({ line, processEditable = false, savingProcessKey = '', savingFooterKey = '', onProcessValueChange, onFooterValueChange }: { line: JobCardLine; processEditable?: boolean; savingProcessKey?: string; savingFooterKey?: string; onProcessValueChange?: (processName: string, field: ProcessField, value: string) => void; onFooterValueChange?: (field: FooterField, value: string) => void }) {
  const attributes = readAttributes(line)
  ...
  return <article className="job-card-sheet">
    ...
    <table className="job-card-master"><colgroup><col className="job-card-label-column" /><col className="job-card-value-column" /><col className="job-card-label-column" /><col className="job-card-value-column" /><col className="job-card-label-column" /><col className="job-card-value-column" /><col className="job-card-label-column" /><col className="job-card-value-column" /></colgroup><tbody>
      ...
      <tr className="job-card-creasing-row"><td colSpan={8}><dl className="job-card-creasing-summary">
        <div><dt>Slotting Size</dt><dd>{slottingSize == null ? '' : `${numberText(slottingSize)} mm`}</dd></div>
        <div><dt>Board / Creasing Allowance</dt><dd>{attributes.board_creasing_allowance ? `${attributes.board_creasing_allowance} mm` : ''}</dd></div>
        <div><dt>Adjusted Width</dt><dd>{rotaryCalculation ? `${numberText(rotaryCalculation.adjustedWidth)} mm` : ''}</dd></div>
        <div><dt>Top Flap</dt><dd>{rotaryCalculation ? `${numberText(rotaryCalculation.topFlap)} mm` : ''}</dd></div>
        <div><dt>Box Height</dt><dd>{rotaryCalculation ? `${numberText(line.height_mm)} mm` : ''}</dd></div>
        <div><dt>Bottom Flap</dt><dd>{rotaryCalculation ? `${numberText(rotaryCalculation.bottomFlap)} mm` : ''}</dd></div>
      </dl></td></tr>
    ...
    <table className="job-card-process"><colgroup><col className="job-card-col-serial" /><col className="job-card-col-process" /><col className="job-card-col-time" /><col className="job-card-col-time" /><col className="job-card-col-reel" /><col className="job-card-col-weight" /><col className="job-card-col-weight" /><col className="job-card-col-weight" /><col className="job-card-col-qty" /><col className="job-card-col-qty" /><col className="job-card-col-employee" /></colgroup><thead><tr><th>S.N</th><th>Process</th><th>Start Datetime</th><th>End Datetime</th><th>Reel No.</th><th>In Reel Weight</th><th>Out Reel Weight</th><th>Remaining Reel Weight</th><th>In Qty</th><th>Out Qty</th><th>Emp. Name</th></tr></thead><tbody>
      {(stages.length ? stages : ['Paper Cutting', 'Corrugation', 'Pasting', 'Quality Inspection']).map((stage, index) => {
        ...
        <ReelDetailCell stage={stage} label="Remaining Reel Weight" entry={entry} field="remaining_reel_weight" secondField="remaining_reel_weight_2" numeric processEditable={processEditable} savingProcessKey={savingProcessKey} onChange={onProcessValueChange} />
        <td className={stage === 'Corrugation' ? 'job-card-split-process-values' : ''}><div className={stage === 'Corrugation' ? 'job-card-process-entry-stack' : ''}>{processEditable ? <><input key={`${stage}:in:${entry?.in_quantity ?? ''}`} className="job-card-process-entry" aria-label={`${stage} In Qty 1`} inputMode="decimal" defaultValue={entry?.in_quantity ?? ''} disabled={savingProcessKey === `${stage}:in_quantity`} onBlur={(event) => onProcessValueChange?.(stage, 'in_quantity', event.target.value)} />{stage === 'Corrugation' && <input key={`${stage}:in2:${entry?.in_quantity_2 ?? ''}`} className="job-card-process-entry" aria-label="Corrugation In Qty 2" inputMode="decimal" defaultValue={entry?.in_quantity_2 ?? ''} disabled={savingProcessKey === `${stage}:in_quantity_2`} onBlur={(event) => onProcessValueChange?.(stage, 'in_quantity_2', event.target.value)} />}</> : <><span>{numberText(entry?.in_quantity)}</span>{stage === 'Corrugation' && <span>{numberText(entry?.in_quantity_2)}</span>}</>}</div></td>
        <td className={stage === 'Corrugation' ? 'job-card-split-process-values' : ''}><div className={stage === 'Corrugation' ? 'job-card-process-entry-stack' : ''}>{processEditable ? <><input key={`${stage}:out:${entry?.out_quantity ?? ''}`} className="job-card-process-entry" aria-label={`${stage} Out Qty 1`} inputMode="decimal" defaultValue={entry?.out_quantity ?? ''} disabled={savingProcessKey === `${stage}:out_quantity`} onBlur={(event) => onProcessValueChange?.(stage, 'out_quantity', event.target.value)} />{stage === 'Corrugation' && <input key={`${stage}:out2:${entry?.out_quantity_2 ?? ''}`} className="job-card-process-entry" aria-label="Corrugation Out Qty 2" inputMode="decimal" defaultValue={entry?.out_quantity_2 ?? ''} disabled={savingProcessKey === `${stage}:out_quantity_2`} onBlur={(event) => onProcessValueChange?.(stage, 'out_quantity_2', event.target.value)} />}</> : <><span>{numberText(entry?.out_quantity)}</span>{stage === 'Corrugation' && <span>{numberText(entry?.out_quantity_2)}</span>}</>}</div></td>
        <td className={stage === 'Corrugation' ? 'job-card-split-process-values' : ''}><div className={stage === 'Corrugation' ? 'job-card-process-entry-stack' : ''}>{processEditable ? <><input key={`${stage}:employee:${entry?.employee_name ?? ''}`} className="job-card-process-entry" aria-label={`${stage} Employee Name 1`} maxLength={120} defaultValue={entry?.employee_name ?? ''} disabled={savingProcessKey === `${stage}:employee_name`} onBlur={(event) => onProcessValueChange?.(stage, 'employee_name', event.target.value)} />{stage === 'Corrugation' && <input key={`${stage}:employee2:${entry?.employee_name_2 ?? ''}`} className="job-card-process-entry" aria-label="Corrugation Employee Name 2" maxLength={120} defaultValue={entry?.employee_name_2 ?? ''} disabled={savingProcessKey === `${stage}:employee_name_2`} onBlur={(event) => onProcessValueChange?.(stage, 'employee_name_2', event.target.value)} />}</> : <><span>{entry?.employee_name}</span>{stage === 'Corrugation' && <span>{entry?.employee_name_2}</span>}</>}</div></td></tr>})}
    </tbody></table>
    <footer>
      {([['supervisor_name', 'Supervisor', false], ['quality_name', 'Quality', false], ['dispatch_name', 'Dispatch', false], ['box_weight_kg', 'Box Wt. (Kg)', true], ['manufactured_quantity', 'Manufactured Qty', true]] as const).map(([field, label, numeric]) => <label key={field}><span>{label}</span>{processEditable ? <input key={`${field}:${line[field] ?? ''}`} className="job-card-footer-entry" aria-label={label} inputMode={numeric ? 'decimal' : undefined} maxLength={numeric ? undefined : 120} defaultValue={line[field] ?? ''} disabled={savingFooterKey === field} onBlur={(event) => onFooterValueChange?.(field, event.target.value)} /> : <strong>{numeric ? numberText(line[field] as number | null | undefined) : line[field]}</strong>}</label>)}
    </footer>
  </article>
}
