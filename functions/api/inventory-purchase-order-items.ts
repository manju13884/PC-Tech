import { getZohoInventoryPurchaseOrderItems } from '../../lib/inventoryProcurement'
...
export async function onRequestGet(context: Context): Promise<Response> {
  ...
  const purchaseOrderId = new URL(context.request.url).searchParams.get('purchase_order_id')?.trim() ?? ''
  if (!purchaseOrderId) return response({ error: 'Select PO Number first.' }, 400)
  try {
    return response(await getZohoInventoryPurchaseOrderItems(purchaseOrderId, context.env))
  } catch (error) {
    console.error('[inventory-purchase-order-items] Unable to load Zoho Books purchase order items', error)
    ...
    return response({ error: 'Unable to load items for the selected purchase order from Zoho Books.' }, 502)
  }
}
